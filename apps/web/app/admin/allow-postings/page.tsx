...
export default function AllowPostingsPage() {
    ...
    const [selected, setSelected] = useState<AdminFreePostingGrant | null>(
        null,
    );
    ...
    const load = useCallback(async () => {
        ...
        setError(null);
        try {
            const res = await adminApi.list_free_posting_grants({
                pageSize: 100,
            });
            setGrants(res.items);
            setTotal(res.total);
            ...
            <section className="rounded-xl border border-border bg-card overflow-hidden">
                ...
                {error ? (
                    ...
                    <div className="px-5 py-14 text-center text-[12.5px] text-muted-foreground">
                        {q
                            ? "No grants match that search."
                            : "No grants yet. Use the form above."}
                    </div>
                ) : (
...
function GrantForm({ onCreated }: { onCreated: () => void }) {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        if (isNaN(n) || n < 1) {
            setErr("Enter a valid number of postings.");
            return;
        }
        setSaving(true);
        try {
            ...
            setErr(
                e instanceof ApiClientError ? e.message : "Failed to grant.",
            );
        } finally {
            setSaving(false);
        }
        ...
        >
            <div>
                ...
                <p className="text-[12px] text-muted-foreground mt-0.5">
                    Search for a company by name, set the quota, and the founder
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-[11px] text-muted-foreground">
                                                    ...
                                                    {c.freePostingGrants
                                                        .length > 0 &&
                                                        ` · ${c.freePostingGrants.reduce((s, g) => s + g.grantedPostings - g.usedPostings, 0)} granted remaining`}
                                                </p>
                                            </div>
                                        </button>
                                    ))
            ...
            ? [
                  {
                      label: "Revoked by",
                      value: g.revokedBy
                          ? `${g.revokedBy.name ?? "—"} (${g.revokedBy.email ?? "—"})`
                          : "—",
                      colored: "text-rose-500",
                  },
                  {
                      label: "Revoked on",
                      value: formatDateFull(g.revokedAt),
                      colored: "text-rose-500",
                  },
              ]
            : []),
